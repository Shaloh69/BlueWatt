...
export function startCronJobs(): void {
  ...
  cron.schedule('5 * * * *', async () => {
    ...
    try {
      await AggregationService.runHourlyForAllDevices();
    } catch (e) {
      logger.error('[cron] Hourly aggregation failed:', e);
    }
  });
  ...
  cron.schedule('0 4 * * *', async () => {
    ...
    try {
      ...
      logger.error('[cron] Daily aggregation failed:', e);
    }
  });
  ...
  cron.schedule('5 4 1 * *', async () => {
    ...
    try {
      ...
      logger.error('[cron] Monthly aggregation failed:', e);
    }
  });
  ...
  cron.schedule('0 8 * * *', async () => {
    ...
    try {
      await BillingService.markOverdue();
    } catch (e) {
      logger.error('[cron] Mark overdue failed:', e);
    }
  });
  ...
  cron.schedule('0 3 * * 0', async () => {
    ...
    try {
      ...
      logger.info(
        `[cron] Cleanup: ${rawDeleted} raw readings, ${aggDeleted} hourly aggregates deleted`
      );
    } catch (e) {
      logger.error('[cron] Cleanup failed:', e);
    }
  });
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      logger.error('[cron] Relay expiry failed:', e);
    }
  });
