...
async function main() {
  ...
  try {
    ...
    console.log('bill_type note:', e.message);
  }
  ...
  try {
    ...
    console.log('fk_bill_pad note:', e.message);
  }
  ...
  try {
    ...
    console.log('uq_pad_period note:', e.message);
  }
  ...
  try {
    ...
    console.log('re-add fk note:', e.message);
  }
  ...
  try {
    ...
    console.log('uq_pad_period_type note:', e.message);
  }
  ...
  try {
    ...
    console.log('uq_stay_cycle note:', e.message);
  }
  ...
  try {
    ...
    console.log('uq_stay_cycle_type note:', e.message);
  }

  // Verify
  const [cols] = (await pool.execute(
    "SHOW COLUMNS FROM billing_periods LIKE 'bill_type'"
  )) as any[];
  const [idx1] = (await pool.execute(
    "SHOW INDEX FROM billing_periods WHERE Key_name = 'uq_pad_period_type'"
  )) as any[];
  const [idx2] = (await pool.execute(
    "SHOW INDEX FROM billing_periods WHERE Key_name = 'uq_stay_cycle_type'"
  )) as any[];
