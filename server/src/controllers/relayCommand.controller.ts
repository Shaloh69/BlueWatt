...
export const issueRelayCommand = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  ...
  if (!['on', 'off', 'reset'].includes(command)) {
    throw new AppError('Invalid command. Use on, off, or reset.', HTTP_STATUS.BAD_REQUEST, ERROR_CODES.VALIDATION_ERROR);
  }
...
export const getPendingCommand = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  const deviceId = req.deviceId;
  if (!deviceId) throw new AppError('Device not identified', HTTP_STATUS.UNAUTHORIZED, ERROR_CODES.UNAUTHORIZED);
...
export const ackRelayCommand = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  const deviceId = req.deviceId;
  if (!deviceId) throw new AppError('Device not identified', HTTP_STATUS.UNAUTHORIZED, ERROR_CODES.UNAUTHORIZED);

  const { command_id, relay_status } = req.body as { command_id: number; relay_status: 'on' | 'off' | 'tripped' };
...
export const getCommandHistory = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  const deviceId = parseInt(req.params.id, 10);
