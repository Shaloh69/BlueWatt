...
export const listTenants = asyncHandler(async (_req: Request, res: Response, _next: NextFunction) => {
 ...
 */
export const createTenant = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  if (!req.user) throw new AppError('Unauthenticated', HTTP_STATUS.UNAUTHORIZED, ERROR_CODES.UNAUTHORIZED);

  const {
    email, full_name, password,
    pad_name, rate_per_kwh, device_id,
  } = req.body as {
    email: string;
    full_name: string;
    password: string;
  ...
  if (!email?.trim() || !full_name?.trim() || !password) {
    throw new AppError('email, full_name and password are required', HTTP_STATUS.BAD_REQUEST, ERROR_CODES.VALIDATION_ERROR);
  }
...
export const deleteTenant = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
  ...
  if (user.role === 'admin') throw new AppError('Cannot delete an admin account', HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
