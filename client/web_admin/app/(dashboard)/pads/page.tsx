    ...
    try {
      await padsApi.assign(showAssign.id, {
        tenant_id: assignForm.tenant_id
          ? parseInt(assignForm.tenant_id)
          : undefined,
        device_id: assignForm.device_id
          ? parseInt(assignForm.device_id)
          : undefined,
      });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Tooltip
            delay={3000}
            content="Create a new rental unit/pad"
            placement="bottom"
          >
            <Button
              color="primary"
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <div className="flex gap-1">
                          ...
                          <Tooltip
                            delay={3000}
                            content="Assign tenant or device to this pad"
                            placement="top"
                          >
                            <Button
                              size="sm"
                          ...
                          {(p.tenant_id || p.device_id) && (
                            <Tooltip
                              delay={3000}
                              content="Remove tenant and device assignment"
                              placement="top"
                              color="danger"
                            >
                              <Button
                          ...
                          <Tooltip
                            delay={3000}
                            content="Permanently delete this pad"
                            placement="top"
                            color="danger"
                          >
                            <Button
        ...
        <ModalContent>
          ...
          <ModalBody className="space-y-3">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, rate_per_kwh: e.target.value }))
              }
            />
          </ModalBody>
          <ModalFooter>
        ...
        <ModalContent>
          ...
          <ModalBody className="space-y-3">
            ...
            <Input
              ...
              type="number"
              step="0.01"
              value={editForm.rate_per_kwh}
              onChange={(e) =>
                setEditForm((f) => ({ ...f, rate_per_kwh: e.target.value }))
              }
          ...
          <ModalFooter>
            ...
            <Button color="primary" isLoading={saving} onPress={handleEdit}>
              Save Changes
            </Button>
          </ModalFooter>
        </ModalContent>
          ...
          <ModalBody>
            ...
            {(deleteTarget?.tenant_id || deleteTarget?.device_id) && (
              <p className="text-sm text-warning mt-1">
                This pad has an active assignment. Deleting it will unlink the
                tenant and device.
              </p>
            )}
      ...
      <Modal
        isOpen={!!showAssign}
        onOpenChange={() => setShowAssign(null)}
        classNames={modalClassNames}
      >
        <ModalContent>
          ...
          <ModalBody className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs text-default-500 font-medium uppercase tracking-wide">
                Tenant
              </label>
              <select
                value={assignForm.tenant_id}
                onChange={(e) =>
                  setAssignForm((f) => ({ ...f, tenant_id: e.target.value }))
                }
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                onChange={(e) =>
                  setAssignForm((f) => ({ ...f, device_id: e.target.value }))
                }
                className="w-full rounded-xl border border-default-200 bg-default-100 px-3 py-2.5 text-sm text-foreground focus:outline-none focus:border-primary"
              >
          ...
          <ModalFooter>
            <Button variant="flat" onPress={() => setShowAssign(null)}>
              Cancel
            </Button>
            <Button color="primary" isLoading={saving} onPress={handleAssign}>
              Save
            </Button>
          </ModalFooter>
        </ModalContent>
