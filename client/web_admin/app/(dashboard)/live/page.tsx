...
export default function LivePage() {
  ...
  const connectSSE = useCallback(() => {
    ...
    const token =
      typeof window !== "undefined" ? localStorage.getItem("bw_token") : null;
          ...
          <CardBody className="flex flex-row items-center gap-4 py-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-3xl font-bold text-purple-400 leading-tight">
                {reading?.energy_kwh != null
                  ? Number(reading.energy_kwh).toFixed(4)
                  : "—"}
          ...
          <CardBody className="flex flex-row items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                ...
                color="success"
                variant="flat"
                isLoading={relayPending}
                onPress={() => handleRelay("on")}
              >
                ON
              </Button>
              <Button
      ...
      {reading ? (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {metricCard(
              "Voltage",
              Number(reading.voltage_rms).toFixed(1),
              "V",
              "text-blue-400",
            )}
            {metricCard(
              "Current",
              Number(reading.current_rms).toFixed(3),
              "A",
              "text-green-400",
            )}
            {metricCard(
              "Active Power",
              Number(reading.power_real).toFixed(1),
              "W",
              "text-yellow-400",
            )}
            {metricCard(
              "Apparent Power",
              Number(reading.power_apparent).toFixed(1),
              "VA",
            )}
            {metricCard(
              "Power Factor",
              Number(reading.power_factor).toFixed(2),
              "",
            )}
            {metricCard(
              "Frequency",
              Number(reading.frequency ?? 0).toFixed(1),
              "Hz",
            )}
          </div>
          ...
          <CardBody className="p-0">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-default-200">
              ...
              <span className="text-xs text-default-400 ml-auto">
                last {MAX_LOG} entries · newest first
              </span>
            </div>
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-content1 z-10">
                  <tr className="border-b border-default-200">
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left py-2 px-3 text-default-500 font-medium uppercase tracking-wide whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {readingLog.map((r, i) => (
                    <tr
                      key={i}
                      className="border-b border-default-100 hover:bg-default-50 font-mono"
                    >
                      <td className="py-1.5 px-3 text-default-400 whitespace-nowrap">
                        {new Date(r.timestamp).toLocaleTimeString()}
                      </td>
                      <td className="py-1.5 px-3 text-blue-400">
                        {Number(r.voltage_rms).toFixed(1)}
                      </td>
                      <td className="py-1.5 px-3 text-green-400">
                        {Number(r.current_rms).toFixed(3)}
                      </td>
                      <td className="py-1.5 px-3 text-yellow-400">
                        {Number(r.power_real).toFixed(1)}
                      </td>
                      <td className="py-1.5 px-3">
                        {Number(r.power_apparent).toFixed(1)}
                      </td>
                      <td className="py-1.5 px-3">
                        {Number(r.power_factor).toFixed(3)}
                      </td>
                      <td className="py-1.5 px-3">
                        {r.frequency != null
                          ? Number(r.frequency).toFixed(1)
                          : "—"}
                      </td>
                      <td className="py-1.5 px-3 text-primary">
                        {r.energy_kwh != null
                          ? Number(r.energy_kwh).toFixed(4)
                          : "—"}
                      </td>
                    </tr>
                  ))}
