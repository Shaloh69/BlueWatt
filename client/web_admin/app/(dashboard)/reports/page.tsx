...
export default function ReportsPage() {
  ...
  const { data: daily = [], isLoading: loading } = useDailyReport(
    selectedDevice,
    month,
  );
  ...
  const totalKwh = daily.reduce(
    (s: number, d: DailyAggregate) => s + Number(d.total_energy_kwh),
    0,
  );
          ...
          {loading ? (
            ...
            <div className="h-64 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-default-400">
                {selectedDevice ? "No data for this period" : "Select a device"}
              </p>
            </div>
          ) : (
                  ...
                  {daily.map((d: DailyAggregate) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-3 font-mono text-xs">
                        {Number(d.total_energy_kwh).toFixed(3)}
                      </td>
                      <td className="py-2 px-3 font-mono text-xs">
                        {Number(d.avg_power_real).toFixed(1)}
                      </td>
                      <td className="py-2 px-3 font-mono text-xs">
                        {Number(d.avg_voltage).toFixed(1)}
                      </td>
